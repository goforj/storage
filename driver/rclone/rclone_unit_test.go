...
func TestRcloneWrapError(t *testing.T) {
	if err := wrapError(fs.ErrorObjectNotFound); !errors.Is(err, filesystem.ErrNotFound) {
		t.Fatalf("expected ErrNotFound, got %v", err)
	}
	if err := wrapError(fs.ErrorPermissionDenied); !errors.Is(err, filesystem.ErrForbidden) {
		t.Fatalf("expected ErrForbidden, got %v", err)
	}
	if err := wrapError(hash.ErrUnsupported); !errors.Is(err, filesystem.ErrUnsupported) {
